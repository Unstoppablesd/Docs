...
>
> :cat: `非阻塞赋值（<=）`：电路结构和触发沿`有`关系，只有在触发沿的时刻才能进行阻塞赋值；在赋值开始时刻计算`<=`右边的语句值，赋值结束时刻更新`<=`左边的语句值（这个过程为两次相同的时钟边沿两次上升沿或者两次下降沿）且此过程允许其它的赋值语句并行执行；
...
> [!tip]
>
> :cat: 计数器在数字系统中主要是对脉冲的个数进行计数，可以实现测量、计数、控制分频等功能；
>
> :cat: 计数器可在电子计算机的控制器中对指令地址进行计数，以便顺序取出下一条指令；在运算器中作乘法、除法运算时记下加减法的次数，在数字仪器中对脉冲计数等；
>
> :cat: 下方实现一个计数器，使得`LED`灯亮`0.5s`熄灭`0.5s`交替闪烁，`sys_clk`来自外部晶振输入，时钟频率为`50Mhz`；

* 计数器实验框图：

![image-20260203192242683](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260203192242683.png)

* 完整的位宽时序图：

![image-20260203192359652](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260203192359652.png)

* 节省位宽的时序图：

![image-20260203192616957](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260203192616957.png)
...
/* 不同值要分开在两个 always 语句之中 */
always@(posedge sys_clk or negedge sys_rst) begin
    if (1'b0 == sys_rst)
        /* 异步复位，非阻塞赋值，等待下一个上升沿修改值 */
        tmp <= 25'b0;
    else if (CNT == tmp)
        tmp <= 25'b0;
    else
...
* 引入中间变量（脉冲信号），此种实现方法可以使代码更简洁易懂：

![image-20260203211335556](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260203211335556.png)
...
/* led */
/* led_out 直接检测 flag 的边沿变化即可 */
always@(negedge sys_rst or negedge flag) begin
    if (1'b0 == sys_rst)
        led_out <= 1'b0;
    else if (1'b0 == flag)
        led_out <= ~led_out;
...
> [!tip]
>
> :cat: 分频器是数字系统设计中最常见的基本电路之一，实现的功能是把输入信号的频率变成成倍数的低于输入频率的输出信号；
>
> :cat: 分频器分为偶数分频器（2，4，6，.....）和奇数分频器（1，3，5， .....）；
...
![image-20260208140142277](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208140142277.png)

* 版本1：占空比不是`50%`的分频方法，总周期为5倍分频：

![image-20260208140333177](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208140333177.png)
...
![image-20260208141323313](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208141323313.png)
...
![image-20260208162803215](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208162803215.png)

* 版本3：输出占空比为`50%`的方波：

![image-20260208163022243](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208163022243.png)
...
/* 使用 assign 语句 即组合逻辑进行赋值，这样不会延后一个时钟周期 */
/* 就是不用寄存器，即 D 触发器 */
...
![image-20260208175109645](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260208175109645.png)

#### 13. 按键消抖

> [!tip]
>
> :cat: 按键消抖分为`软件消抖`和`硬件消抖`两部分，最常用成本最低的就是`软件消抖`；常见的硬件消抖方式为使用`RS触发器`；
>
> :cat: 按下按键一次抖动时间一般为`10ms`左右，所以使用这个经验值进行抖动检测；
>
> :cat: 具体实现方法为使用计数器实现，当检测到低电平的时候计数器进行计数，当检测到高电平的时候计数器的值进行清零；

![image-20260212102246960](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212102246960.png)

![image-20260212104650367](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212104650367.png)

![image-20260212110403523](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212110403523.png)

![image-20260212111057417](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212111057417.png)

![image-20260212111553863](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212111553863.png)
...
module fliter
#(
    parameter count = 20'd999_999
)(
    input wire [0:0] sys_clk,
    input wire [0:0] sys_rst,
    input wire [0:0] key_in,

    output reg [0:0] key_out
);
...
![image-20260212121824692](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212121824692.png)

#### 14. 流水灯

> [!tip]

![image-20260212133023662](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212133023662.png)

![image-20260212145300436](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212145300436.png)

![image-20260212151053514](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212151053514.png)
...
module flow_led
#(
    parameter count = 25'd24_999_999
)(
    input wire [0:0] sys_clk,
    input wire [0:0] sys_rst,
...
always@(posedge sys_clk or negedge sys_rst) begin
    if (1'b0 == sys_rst)
        led_out <= 4'b0001;
    /* 此处必须先进行赋值，如果先执行移位就只能执行一次再也无法满足条件了 */
    else if ((4'b1000 == led_out) && (1'b1 == flag))
        led_out <= 4'b0001;
    else if (1'b1 == flag)
        led_out <= (led_out << 1);
    else
...
> [!tip]
>
> :cat: 呼吸灯的工作原理是固定一个周期比如`1s`，通过将`1s`分成若干份比如`1000`份（分的越多则呼吸灯的效果越细腻），通过控制`1000`份中的高电平占空比来控制led灯的亮度变化；

![image-20260212201646701](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212201646701.png)

![image-20260212205921938](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260212205921938.png)
...
module breath
#(
    parameter COUNT_US_MAX = 6'd49,
    parameter COUNT_MS_MAX = 10'd1000,
    parameter COUNT_S_MAX = 10'd1000
)(
    input wire [0:0] sys_clk,
    input wire [0:0] sys_rst,
...
always@(posedge sys_clk or negedge sys_rst) begin
    if (1'b0 == sys_rst)
        led_out <= 1'b0;
    else if ((cnt_ms <= cnt_s) && (1'b0 == flag))
        led_out <= 1'b1;
    else if ((cnt_ms <= cnt_s) && (1'b1 == flag))
        led_out <= 1'b0;
    else if ((cnt_ms > cnt_s) && (1'b0 == flag))
        led_out <= 1'b0;
    else if ((cnt_ms > cnt_s) && (1'b1 == flag))
...
![image-20260213100731533](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260213100731533.png)

#### 16. 状态机

> [!tip]
>
> :cat: 状态机`FSM`(Finite State Machine)，也称为`同步`有限状态机，一般简称状态机；
>
> :cat: 状态机可以分为`Moore`型状态机和`Mealy`型状态机；其中二者均为`状态跳转`且只和输入有关，区别主要在输出时，`Moore`型状态机输出只和当前状态有关和输入是无关的，`Mealy`状态机输出不仅和当前的状态有关还和输入有关；
>
> :cat: `FSM`状态定义一般有三种常用的方法如：`格雷码`、`二进制编码`、`独热码`；在实际项目中如果状态`小于4个`就用`二进制编码`，如果状态在`4 - 24`之间就使用`独热码`，如果状态数量`大于24`则直接使用`格雷码`即可；但是如果在高速系统中，必须优先使用`独热码`，否则会出现很严重的后果；
...
/* 格雷码 */
/* 两种状态之间相邻的几位中只能有一位不同其余均要相同 */
parameter STATE1 = 2'b01;
parameter STATE2 = 2'b11;
parameter STATE3 = 2'b10;
```

* 状态机分类如下所示`Moore`&`Mealy`两类，以投币机作为例子进行展示：

![image-20260213110839773](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260213110839773.png)

![image-20260213110904153](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260213110904153.png)

* 实验1：简单`投币机`实验，每投入`3`枚硬币输出一次数据；

![image-20260213110208206](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260213110208206.png)

![image-20260213112038976](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260213112038976.png)
...
module fsm(
    input wire [0:0] sys_clk,
    input wire [0:0] sys_rst,
    input wire [0:0] fsm_in,

    output reg [0:0] fsm_out
);

/* 使用热独码定义状态值 */
parameter IDLE = 3'b001;
parameter ONE_COIN = 3'b010;
parameter TWO_COIN = 3'b100;
...
always@(posedge sys_clk or negedge sys_rst) begin
    if (1'b0 == sys_rst)
        fsm_out <= 1'b0;
    else if ((TWO_COIN == status) && (1'b1 == fsm_in))
        fsm_out <= 1'b1;
    else 
        fsm_out <= 1'b0;
end
...
/* 定义探针寄存器用于打印状态信息 */
/* 注意这里一定要是 wire 类型，否则无法打印数据 */
wire [2:0] tb_status = fsm_inst.status;
...
initial begin
    $timeformat(-9, 0, "ns", 6);
    $monitor("time: %t tb_fsm_in:%b status:%b tb_fsm_out:%b\n", $time, tb_fsm_in, tb_status, tb_fsm_out);
end

always #10 tb_sys_clk <= ~tb_sys_clk;

always@(posedge tb_sys_clk or negedge tb_sys_rst) begin 
    if (1'b0 == tb_sys_rst)
        tb_fsm_in <= 1'b0;
    else 
        tb_fsm_in <= {$random} % 2;
...
![image-20260214110740421](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260214110740421.png)

* 实验2：复杂一些的`投币机`实验，设定每`2.5`元输出一个时钟周期的高电平，投币金额有`1元`和`0.5`元两种；

![image-20260214114958414](D:\Workspace\Docs\FPGA\Xilinx\assets\image-20260214114958414.png)
